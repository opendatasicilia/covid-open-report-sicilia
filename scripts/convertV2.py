...
def get_pages(file):
    ...
    for page in range(len(pdf_file)):
        ...
        if "ALLEGATO" in pdf_text:
            book.append(page)
    incidence_pages = list(range(book[0], book[1] + 1))
    vaccines_pages = list(range(book[1] + 1, len(pdf_file) + 1))
    output = {
        "data": latest_date,
...
def add_to_readme():
    ...
    with open("./README.md", "r+", encoding="utf-8") as readme:
        ...
        if last_day != new_day:
            insert_content = (
                f'<li><a href="{latest_url}">Report {format_date}.pdf</a></li>\n'
            )
...
def get_incidence(file, input_pages):
    ...
    pdf = fitz.open(file)
    all_text = []
    for index in range(input_pages[0], input_pages[-1]):
    ...
    # Esporta CSV
    last_update = (
        open(f"{PATH_INCIDENCE}.csv", encoding="UTF-8")
        .read()
        .rsplit("\n", 2)[1]
        .split(",")[0]
    )
    if latest_date != last_update:
        ...
        output.to_csv(
            f"{PATH_INCIDENCE}-{format_date}.csv",
            index=None,
            header=True,
        )
        output.to_csv(f"{PATH_INCIDENCE}-latest.csv", index=None, header=True)
        output.to_csv(
            f"{PATH_INCIDENCE}.csv",
            mode="a",
            index=None,
            header=False,
        )
    format_date = latest_date.replace("-", "")
    return f"{PATH_INCIDENCE}-{format_date}.csv"
...
def get_vaccines(file, input_pages):
    ...
    report = pd.DataFrame(
        pd.read_csv(vaccini_latest, converters={"pro_com_t": "{:0>6}".format})
    )
    comuni = []
    values = []
    ...
    for index in vaccini_pages:
        ...
        results = pytesseract.image_to_string(
            processed_comuni, config=letters_config
        ).split("\n")
        ...
        for word in lista_risultati:
            for i, comune in enumerate(lista_comuni):
                if similar(word, comune) == 1:
                    ...
                    del lista_comuni[i]
                    break
                elif similar(word, comune) > 0.85:
                    del lista_comuni[i]
                    comuni.append(comune)
                    break
                elif similar(word, comune) > 0.66:
                    del lista_comuni[i]
                    comuni.append(comune)
                    break
                elif similar(word, comune) > 0.57:
                    del lista_comuni[i]
                    comuni.append(comune)
                    break
                else:
        ...
        results = pytesseract.image_to_string(
            processed_numbers, config=numbers_config
        ).split("\n")
    ...
    if len(comuni) == 390 and len(values) == 390:
        ...
        print("nope")
        print(len(comuni), len(values))
        exit()

    vax = pd.DataFrame(vax_tuples, columns=["comune", "prima_dose", "seconda_dose"])
    output = pd.merge(vax, comuni_siciliani, on="comune", how="inner")
    output = output[
        ["cod_prov", "pro_com_t", "provincia", "comune", "prima_dose", "seconda_dose"]
    ]
    output.insert(0, "data", latest_date)
    assert len(output) == 390, "Errore: Sono presenti meno comuni del previsto."
    last_update = (
        open(f"{PATH_VACCINES}.csv", encoding="UTF-8")
        .read()
        .rsplit("\n", 2)[1]
        .split(",")[0]
    )
    if latest_date != last_update:
        ...
        format_date = latest_date.replace("-", "")
        output.to_csv(
            f"{PATH_VACCINES}-{format_date}.csv",
            index=None,
            header=True,
        )
        output.to_csv(f"{PATH_VACCINES}-latest.csv", index=None, header=True)
        output.to_csv(f"{PATH_VACCINES}.csv", mode="a", index=None, header=False)
    return f"{PATH_VACCINES}-{format_date}.csv"
